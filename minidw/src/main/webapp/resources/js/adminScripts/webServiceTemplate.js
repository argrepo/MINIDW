var webServiceAuthentication = {
		 ...
		 validateOAuth1WebServiceTemplate : function(){
			 var validStatus = true;
			 var requestURL = $('#oAuth1\\.requestURL').val();
			 var tokenURL = $('#oAuth1\\.tokenURL').val();
			 var authURL = $('#oAuth1\\.authURL').val();
			 var consumerKey = $("#oAuth1\\.consumerKey").val();
			 var consumerSecret = $("#oAuth1\\.consumerSecret").val();
			 var version = $("#oAuth1\\.version").val();
			 var signatureMethod = $("#oAuth1\\.signatureMethod  option:selected").val();
			 var callbackURL = $("#oAuth1\\.callbackURL").val();
			 
			 if(requestURL == ''){
				 common.showcustommsg("#oAuth1\\.requestURL", globalMessage['anvizent.package.message.enterrequestURL'], "#oAuth1\\.requestURL");
				 validStatus = false;
			 }
			 if(tokenURL == ''){
				 common.showcustommsg("#oAuth1\\.tokenURL", globalMessage['anvizent.package.message.entertokenURL'], "#oAuth1\\.tokenURL");
				 validStatus = false;
			 }
			 if(authURL == 0){
				 common.showcustommsg("#oAuth1\\.authURL", globalMessage['anvizent.package.message.enterauthURL'], "#oAuth1\\.authURL");
				 validStatus = false;
			 }
			 if(consumerKey == ''){
					common.showcustommsg("#oAuth1\\.consumerKey",globalMessage['anvizent.package.message.enterconsumerKey'], "#oAuth1\\.consumerKey");
					validStatus = false;
				}
			if(consumerSecret == ''){
				common.showcustommsg("#oAuth1\\.consumerSecret",globalMessage['anvizent.package.message.enterconsumerSecret'], "#oAuth1\\.consumerSecret");
				validStatus = false;
			}
			if(version == ''){
				common.showcustommsg("#oAuth1\\.version",globalMessage['anvizent.package.message.version'], "#oAuth1\\.version");
				validStatus = false;
			}
			if(signatureMethod == ''){
				common.showcustommsg("#oAuth1\\.signatureMethod",globalMessage['anvizent.package.message.entersignatureMethod'], "#oAuth1\\.signatureMethod");
				validStatus = false;
			}
			if(callbackURL == ''){
				common.showcustommsg("#oAuth1\\.callbackURL",globalMessage['anvizent.package.label.enterCallBackUrl'], "#oAuth1\\.callbackURL");
				validStatus = false;
			}
...
if($(".webServiceTemplate-page").length){
	...
	if(authTypeid == 2){
		...
		$(".authView").show();
		$(".oAuth1RequestURL,.oAuth1TokenURL,.oAuth1AuthURL,.oAuth1ConsumerKey,.oAuth1ConsumerSecret,.oAuth1SignatureMethod,.oAuth1Realm,.oAuth1Scope,.oAuth1Version,.oAuth1CallbackURL").hide();
	}
	if(authTypeid == 4){
		$(".calBackUrl,.url,.oAuthAuthenticationUrl,.accessTokenUrl,.webServicesCoveringDiv, .clientIdentifier,.clientSecret,.scope,.state").hide();
		$('.baseUrlDiv,.loginurl, .logouturl').hide();  
		$(".authView,.baseUrlRequiredForAuthentication,.methodType,.requestParams,#apiBodyParams").show();
		$(".oAuth1RequestURL,.oAuth1TokenURL,.oAuth1AuthURL,.oAuth1ConsumerKey,.oAuth1ConsumerSecret,.oAuth1SignatureMethod,.oAuth1Realm,.oAuth1Scope,.oAuth1Version,.oAuth1CallbackURL").show(); 
	}
	...
	$("#webServiceAuthenticationTypes\\.id").on("change",function(){
		common.clearValidations(["#webServiceName,#authenticationUrl,#authenticationMethodType,.methodTypeSelectionValidation,#oAuth2\\.redirectUrl," +
				...
				"#webServiceAuthenticationTypes\\.id,#dateFormat,#timeZone," +
				"#baseUrl","#oAuth1\\.oAuth1RequestURL","#oAuth1\\.oAuth1TokenURL","#oAuth1\\.oAuth1AuthURL","#oAuth1\\.oAuth1ConsumerKey","#oAuth1\\.oAuth1ConsumerSecret","#oAuth1\\.oAuth1SignatureMethod","#oAuth1\\.oAuth1Realm","#oAuth1\\.oAuth1Scope","#oAuth1\\.oAuth1Version","#oAuth1\\.oAuth1CallbackURL"]);
		 $("#authenticationUrl,#oAuth2\\.redirectUrl,#oAuth2\\.accessTokenUrl,#oAuth2\\.authUrl,.requestParam,.oAuthRequestParam,.authRequestKey," +
				 ".authRequestValue,.authreqheaders,.authBodyValue,.authBodyKey,.url," +
				 "#baseUrl,#oAuth1\\.requestURL,#oAuth1\\.tokenURL,#oAuth1\\.authURL,#oAuth1\\.consumerKey,#oAuth1\\.consumerSecret,#oAuth1\\.signatureMethod,#oAuth1\\.realm,#oAuth1\\.scope,#oAuth1\\.version,#oAuth1\\.callbackURL").val("");
...
$(document).on("click","#saveWebserviceTemp",function(){
	...
	common.clearValidations(["#webServiceName,#authenticationUrl,#authenticationMethodType,.methodTypeSelectionValidation,#oAuth2\\.redirectUrl," +
		"#oAuth2\\.accessTokenUrl,#OAuth2\\.grantType,.requestParam,#oAuth2\\.authUrl,#authRequestKey,#authRequestValue,#authrequestheaders, " +
		"#webServiceAuthenticationTypes\\.id,#dateFormat,#timeZone,#baseUrl,#soapBodyElement,#loginUrl,#logoutUrl"]);
	var status=true;
	var statusTemp=true;
	var authTypeid = $('#webServiceAuthenticationTypes\\.id option:selected').val();
	var webserviceType = $('#webserviceType option:selected').val();
	if(webserviceType == 0){
		common.showcustommsg("#webserviceType",globalMessage['anvizent.package.label.pleaseChooseAuthenticationType'], "#webserviceType");
		return false;
	} 
		if(authTypeid==1 || authTypeid==3){
			 ...
			 status = webServiceAuthentication.validateWebServiceBasicTemplate();
			  ...
			  status = webServiceAuthentication.validateWebServiceBasicTemplate();
			  statusTemp = webServiceAuthentication.validateOAuth1WebServiceTemplate();
		}
