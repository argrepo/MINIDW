...
@Controller
@RequestMapping(value = "/admin/elt/jobtag")
public class ELTJobTagController {
	
	protected static final Log LOGGER = LogFactory.getLog(ELTJobTagController.class);
	...
	public ModelAndView eltConfigTags(HttpServletRequest request, HttpServletResponse response, ModelAndView mv,
		...
		try {
			...
			DataResponse dataResponseConfigTags = eltRestUtilities.getRestObject(request, "/getEltConfigTags", user.getUserId());
			...
			if (dataResponse != null && dataResponse.getHasMessages()) {
				...
				mv.addObject("eltJobsTagsList", dataResponse.getObject());
				mv.addObject("eltConfigTags", dataResponseConfigTags.getObject());
			} else {
				mv.addObject("messagecode", "FAILED");
		...
		mv.setViewName("tiles-anvizent-admin:eltJobTags");
		return mv;
	}
