  ...
  <div class="col-xs-12">
	...
	<div class="alert alert-success successIlMessage" style="display:none;">
	  	...
	  	<table class="table table-striped table-bordered tablebg" id="customPackageTable">
			<thead>
				<tr>
					...
					<th><spring:message code = "anvizent.package.label.merging"/></th>
					<th><spring:message code = "anvizent.package.label.constraints"/></th>
					<th><spring:message code = "anvizent.package.label.sequence"/></th>
		 ...
		 <tbody>
		  	<c:forEach items="${iLConnectionMapping}" var="iLConnectionMapping" varStatus="increment">
		  		<tr>
		  			...
		  			<td><c:out value="${iLConnectionMapping.xReferenceDetails.constraintsCompleted ? 'Completed':'Pending'}"/></td>
		  			<td><c:out value="${iLConnectionMapping.xReferenceDetails.sequenceCompleted ? 'Completed':'Pending'}"/></td>
			      ...
			      data-mappingid="<c:out value="${iLConnectionMapping.connectionMappingId}"/>" class="btn btn-primary btn-sm  xReferenceBtn">X-Ref</button></td>
		  			...
		  			data-mappingid="<c:out value="${iLConnectionMapping.connectionMappingId}"/>" class="btn btn-primary btn-sm <c:out value="${iLConnectionMapping.isFlatFile ? 'flatFilePreviewStandard':'databasePreviewStandard'}"/>"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span></button></td>
		  		</tr>	
		  	</c:forEach>
		  </tbody>
      ...
      <div class="pull-right"> <button type="button" class="btn btn-sm btn-danger" id="closexReferenceDetailsBtn" ><spring:message code = "anvizent.package.button.x"/></button></div>
      	<div>
	      	<button type="button" class="btn btn-primary btn-sm xrefButton mappingBtn" data-divId="mappingDiv-Xref"><spring:message code = "anvizent.package.label.mapping"/></button>
	      	<button type="button" class="btn btn-primary btn-sm xrefButton mergingBtn" data-divId="mergingDiv-Xref"><spring:message code = "anvizent.package.label.merging"/></button>
	      	<button type="button" class="btn btn-primary btn-sm xrefButton constraintBtn" data-divId="constraintDiv-Xref"><spring:message code = "anvizent.package.label.constraints"/></button>
	      	<button type="button" class="btn btn-primary btn-sm xrefButton sequenceBtn" data-divId="sequenceDiv-Xref"><spring:message code = "anvizent.package.button.sequence"/></button>
      	</div>
      ...
      <div class="clearfix" style="height:20px;"></div>
	      ...
	      <div class="row hidden">
			<div class="col-sm-12">
				<div class="pull-right">
					<button class="btn btn-success addConditionGroupBtn" ><spring:message code = "anvizent.package.button.addGroup"/></button>
				</div>
			</div>
		  </div>
	      ...
	      <div class="row groupBoxTemplate hidden">
			...
			<div class="pull-right addColumnBox">
				<button class="btn btn-success addConditionBtn" ><spring:message code = "anvizent.package.button.addCondition"/></button>
				...
				<table class="table table-striped table-bordered tablebg tblForMergingConditions">
					<thead>
						<tr>
							<th>
								<spring:message code = "anvizent.package.label.ilColumnName"/>
							</th>
							<th>
								<spring:message code = "anvizent.package.label.SourceColumnName"/>
							</th>
							<th>
