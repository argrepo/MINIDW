...
@Controller
@RequestMapping(value = "/admin/elt/masterconfig")
public class ELTMasterConfigurationController {
	
	protected static final Log LOGGER = LogFactory.getLog(ELTMasterConfigurationController.class);
	...
	@RequestMapping(value = "/eltMasterConfiguration",method = RequestMethod.GET)
	public ModelAndView eltConfigTags(HttpServletRequest request, HttpServletResponse response, ModelAndView mv,
			HttpSession session,Locale locale) {
		CommonUtils.setActiveScreenName("eltMasterConfiguration", session);
		...
		try {
			...
			if (dataResponse != null && dataResponse.getHasMessages()) {
				...
				mv.addObject("eltMasterConfigurationList", dataResponse.getObject());
			} else {
				mv.addObject("messagecode", "FAILED");
		...
		mv.setViewName("tiles-anvizent-admin:eltMasterConfiguration");
		return mv;
	}
