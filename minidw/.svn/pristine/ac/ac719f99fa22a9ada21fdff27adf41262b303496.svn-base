...
<div class="col-md-12 rightdiv">
	<div class='row form-group'>
		<h4 class="alignText"><spring:message code="anvizent.navLeftTabLink.label.clientTableScriptsExecution"/></h4>
	</div>
...
<c:url value="/admin/clientTableScripts/mapped/listByClient" var="getUrl"/>
<input type="hidden" value="${getUrl}" id="getUrl">
<c:url value="/admin/clientTableScripts/execution" var="executeScripts"/>
...
<form:form modelAttribute="tableScriptsForm" action="${editUrl }"   enctype="multipart/form-data">   
	...
	<c:choose>
		<c:when test="${tableScriptsForm.pageMode == 'list'}">
		   ...
		   <div class="row form-group">
		     <button  type="button" style="float:right; margin-right: 1.5em;" id="executeScripts" class="btn btn-primary btn-sm"><spring:message code="anvizent.package.label.executescripts"/></button></div>
			<div class="col-md-12">
				<table class="table table-striped table-bordered tablebg  dataTable no-footer"   id="tableScriptsMappingTable">
			   <thead>
                <tr>
                <th class="col-xs-1"><input type="checkbox" id="selectall"/> <spring:message code="anvizent.package.label.selectAll"/></th>
                ...
                <th class="col-xs-2"><spring:message code="anvizent.package.label.executionstatus"/></th>
                <th class="col-xs-2"><spring:message code="anvizent.package.label.createdDate"/></th>
                <th class="col-xs-2"><spring:message code="anvizent.package.label.ExecutedDate"/></th>
                </tr> 
                </thead>
                <tbody>
                <c:forEach items="${tableScriptsForm.tableScriptList}" var ="tableScriptList" varStatus="loop">
                    <tr id="tableScriptsRow">
		                 <td>
		                 <input type="checkbox" name="tableScriptList[${loop.index}].checkedScript" id="tableScriptsInfoById${loop.index}" <c:if test="${ tableScriptList.executed == true}">checked="checked" disabled="true"</c:if> class='tableScriptsInfoById checkCase'/>
		                 </td>
		                ...
		                <td >${tableScriptList.scriptName}<input type="hidden" name="tableScriptList[${loop.index}].scriptName" value="${tableScriptList.scriptName}"/><input type="hidden"  name="tableScriptList[${loop.index}].sqlScript" value="${tableScriptList.sqlScript}"/></td> 
                	   ...
                	   ${tableScriptList.executed ? "Completed" : tableScriptList.error ? "Error":"Pending"  }
					  <c:if test="${tableScriptList.executed == false && tableScriptList.error == true }"><input type="button" data-sid="${tableScriptList.id}"  data-sname="${tableScriptList.scriptName}" id="viewErrorScripts" value="View Error" class="btn btn-primary  btn-sm viewErrorScripts"/> 
					  </c:if>
					  <input type="hidden" name="tableScriptList[${loop.index}].executed" value="${tableScriptList.executed}" class='tableScriptsExecutedOrNot checkCase'/>
		               ...
		               <td id="modifiedDate"><c:if test="${empty tableScriptList.modified_Date }">-</c:if><c:if test="${not empty tableScriptList.modified_Date }">${ tableScriptList.modified_Date }</c:if> </td>  
                	</tr>  
                </c:forEach>
                </tbody>
            </table>
					  ...
					  <div class="modal-dialog">
					    <div class="modal-content">
					      ...
					      <div class="modal-body" style="max-height: 400px; overflow-y: auto; overflow-x: hidden;">
										 <div class="form-group">
                                         <label for="comment"><spring:message code="anvizent.package.message.ErrorMsg"/>:</label>
