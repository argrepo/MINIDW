	...
	public File decryptFile(File encryptedFile) throws Exception {
		...
		if (encryptedFile != null) {
			...
			try {
				...
				String dir =  createDir(Constants.Temp.getTempFileDir() + "downloadedFiles/");
				decryptedFile = new File(dir + getFileName(encryptedFile.getAbsolutePath()));
	...
	public File encryptFile(File originalFile) throws Exception {
		...
		if (originalFile != null) {
			...
			try {
				...
				cis = new CipherInputStream(fis, cipher);
				String originalFileName = getFileName(originalFile.getAbsolutePath());
				String dir =  createDir(Constants.Temp.getTempFileDir() + "encryptedFile/");
				...
				LOGGER.info("file encryption completed...." + originalFile);
			} finally {
				if (cis != null)
					cis.close();
				if (fis != null)
