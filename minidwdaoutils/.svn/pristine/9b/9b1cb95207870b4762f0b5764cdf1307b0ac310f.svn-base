					...
					new ResultSetExtractor<WebServiceTemplateMaster>() {
						...
						public WebServiceTemplateMaster extractData(ResultSet rs) throws SQLException, DataAccessException {
							...
							if (rs != null && rs.next()) {
								...
								String oauth1_credentials = rs.getString("oauth1_credentials");
								OAuth1 oAuth1 = null;
								try
								{
									if(oauth1_credentials != null){
									oAuth1 = new ObjectMapper().readValue(oauth1_credentials, OAuth1.class);
									}
									webServiceTemplateMaster.setoAuth1(oAuth1);
								}
								catch ( IOException e )
								{
									e.printStackTrace();
		...
		try {
			...
			updatedCount = clientAppDbJdbcTemplate.update(new PreparedStatementCreator() {
				...
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
					...
					try
					{
						pst.setString(++psCount, new ObjectMapper().writeValueAsString(webServiceTemplateMaster.getoAuth1()));
					}
					catch ( IOException e )
					{
						e.printStackTrace();
		...
		try {
			...
			count = clientAppDbJdbcTemplate.update(sql,
					new Object[] { 
							...
							new ObjectMapper().writeValueAsString(webServiceConnectionMaster.getoAuth1())
			 });
						...
						public WebServiceTemplateMaster extractData(ResultSet rs) throws SQLException {
							if (rs.next()) {
								...
								try
								{
									...
									webServiceTemp.setoAuth1(oAuth1);
								}
								catch ( IOException e )
								{
									e.printStackTrace();
